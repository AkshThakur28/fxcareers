<main>

   <!-- breadcrumb-area-start -->
   <div class="it-breadcrumb-area it-breadcrumb-bg"
      data-background="<?= base_url() ?>public/web/assets/img/breadcrumb/breadcrumb.jpg">
      <div class="container">
         <div class="row ">
            <div class="col-md-12">
               <div class="it-breadcrumb-content z-index-3 text-center">
                  <div class="it-breadcrumb-title-box">
                     <h3 class="it-breadcrumb-title">about us</h3>
                  </div>
                  <div class="it-breadcrumb-list-wrap">
                     <div class="it-breadcrumb-list">
                        <span><a href="<?= base_url() ?>">home</a></span>
                        <span class="dvdr">//</span>
                        <span>about us</span>
                     </div>
                  </div>
               </div>
            </div>
         </div>
      </div>
   </div>
   <!-- breadcrumb-area-end -->

   <!-- about-area-start -->
   <div class="it-about-4-area  inner-about-style pt-80 pb-80">
      <div class="container">
         <div class="row align-items-center">
            <div class="col-xl-6 col-lg-6">
               <div
                  class="it-about-4-thumb-wrap d-flex align-items-center justify-content-center justify-content-lg-end">
                  <div class="it-about-4-thumb-double d-flex flex-column">
                     <img class="mb-20" src="<?= base_url() ?>public/web/assets/img/about/thumb-4-1.jpg" alt="">
                     <img src="<?= base_url() ?>public/web/assets/img/about/thumb-4-2.jpg" alt="">
                  </div>
                  <div class="it-about-4-thumb-single">
                     <img src="<?= base_url() ?>public/web/assets/img/about/thumb-4-3.jpg" alt="">
                  </div>
               </div>
            </div>
            <div class="col-xl-6 col-lg-6">
               <div class="it-about-3-title-box">
                  <span class="it-section-subtitle">
                     <img src="<?= base_url() ?>public/web/assets/img/category/inner.svg" alt="">
                     about us
                  </span>
                  <h2 class="it-section-title-3 pb-30">We Empower Your <span>learning</span> Journey with the best Courses
                  </h2>
                  <p>FXCareers Dubai is a valued member of the FXCareers Global network. We are a
                     respected professional institution dedicated to offering complete training and
                     education in the finance business, with a special focus on Forex and Leading Global
                     Stock Markets, and have been certified by KHDA- Government of Dubai.</p>
                  <p>Along with your official educational degree, we provide one of the top certification
                     courses that will not only help you become a standout trader, but also help you
                     advance your career.</p>
               </div>
               <div class="it-about-3-mv-box">
                  <div class="row">
                     <div class="col-xl-12">
                        <div class="it-about-4-list-wrap d-flex align-items-start">
                           <div class="it-about-4-list-icon">
                              <span><i class="flaticon-video-1"></i></span>
                           </div>
                           <div class="it-about-3-mv-item">
                              <span class="it-about-3-mv-title">Master the Markets</span>
                              <p>Conquer the dynamic world of Forex, stocks,
                                 commodities, and beyond with confidence. Learn from industry leaders and
                                 develop the skills to navigate these markets effectively.</p>
                           </div>
                        </div>
                     </div>
                     <div class="col-xl-12">
                        <div class="it-about-4-list-wrap d-flex align-items-start">
                           <div class="it-about-4-list-icon">
                              <span><i class="flaticon-puzzle"></i></span>
                           </div>
                           <div class="it-about-3-mv-item">
                              <span class="it-about-3-mv-title">Unlock Potential</span>
                              <p>Transform from an aspiring trader to a skilled professional.
                                 FXCareers Dubai equips you with the knowledge and strategies needed to
                                 achieve consistent profitability in the financial markets</p>
                           </div>
                        </div>
                     </div>
                  </div>
               </div>
               <div class="it-about-3-btn-box p-relative">
                  <a class="it-btn" href="<?= base_url() ?>contact">
                     <span>
                        admission open
                        <svg width="17" height="14" viewBox="0 0 17 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                           <path d="M11 1.24023L16 7.24023L11 13.2402" stroke="currentcolor" stroke-width="1.5"
                              stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round" />
                           <path d="M1 7.24023H16" stroke="currentcolor" stroke-width="1.5" stroke-miterlimit="10"
                              stroke-linecap="round" stroke-linejoin="round" />
                        </svg>
                     </span>
                  </a>
                  <div class="it-about-3-left-shape-3 d-none d-xl-block">
                     <img src="<?= base_url() ?>public/web/assets/img/about/about-3-shap-3.png" alt="">
                  </div>
               </div>
            </div>
         </div>
      </div>
   </div>
   <!-- about-area-end -->
   <!-- COO Message -->
   <div class="it-team-area  p-relative pt-80 pb-120">
      <!-- <div class="it-team-shape-1 d-none d-xl-block">
         <img src="assets/img/team/shape-1-1.png" alt="">
      </div> -->

      <div class="container">

         <div class="row">
            <!-- <div class="col-xl-3 col-lg-3">

                  <div class="it-teacher-details-left">
                     <div class="it-teacher-details-left-thumb">
                        <img src="https://fxcareers.ae/uploads/teachers/e48f2a2f3a8a03b365da02a9860c6ad6.jpeg" alt="">
                        <div class="name-details">

                           <h4 class="text-white">Sandesh Nandode</h4>
                           <p class="mb-0 text-white">Chief Operating Officer</p>
                           <p class="mb-0 text-white">FXCAREERS Dubai</p>

                        </div>
                     </div>


                  </div>
               </div> -->
            <div class="col-xl-12 col-lg-12">
               <div class="bg-outer position-relative z-1 p-1">
                  <div class="bg-white bg-inner2">
                     <div class="coo-img">
                        <img src="<?= base_url() ?>public/web/assets/img/sandesh.png" alt="">
                        <div class="coo-d mt-4">
                           <h5>Sandesh Nandode</h5>
                           <p>COO and Chief Mentor</p>
                        </div>
                     </div>

                     <div>
                        <div class="it-team-title-box pb-15">
                           <h4 class="it-section-title">Message from the Chief Operating Officer</h4>
                           <h5 class="font-italic mt-4">Welcome to <span style="font-style: italic; color:var(--it-theme-1)">FXCAREERS!</span></h5>
                        </div>
                        <div class="it-team-text">
                           <p>As the global Chief Operating Officer of FXCAREERS, I am thrilled to share our vision and commitment to transforming financial education worldwide. At FXCAREERS, we specialize in delivering advanced Forex training and insights into global stock markets. However, our mission extends far beyond providing exceptional courses. We aspire to become a full-fledged School of Financial Literacy, dedicated to spreading financial knowledge across the globe.</p>
                           <p>Financial literacy is a cornerstone for improving the standard of living and enhancing the overall well-being of individuals and communities. By equipping people with the skills and knowledge to make informed financial decisions, we aim to empower them to achieve greater financial stability and success.</p>
                           <p>
                              Our team at FXCAREERS is passionate about making financial education accessible to all, regardless of background or experience level. We are committed to providing high-quality, practical training that prepares our students for real-world financial challenges and opportunities.</p>
                           <p>Join us on this exciting journey as we work together to elevate financial literacy and create a brighter future for everyone.</p>
                           <p>Thank you for being a part of FXCAREERS.</p>
                        </div>
                     </div>

                  </div>
               </div>
            </div>
         </div>
      </div>
   </div>
   <!-- funfact-area-start -->
   <div class="it-funfact-area z-index-5">
      <div class="container">
         <div class="it-funfact-bg-wrap theme-bg"
            data-background="<?= base_url() ?>public/web/assets/img/funfact/funfact-bg.png">
            <div class="row gx-0">
               <div class="col-xl-3 col-lg-3 col-md-6 col-sm-6">
                  <div class="it-funfact-item d-flex align-items-center">
                     <div class="it-funfact-icon">
                        <span><i class="flaticon-teacher"></i></span>
                     </div>
                     <div class="it-funfact-content">
                        <h6><i class="purecounter" data-purecounter-duration="1" data-purecounter-end="10">10</i>k+</h6>
                        <span>Successfully Trained</span>
                     </div>
                  </div>
               </div>
               <div class="col-xl-3 col-lg-3 col-md-6 col-sm-6">
                  <div class="it-funfact-item d-flex align-items-center">
                     <div class="it-funfact-icon">
                        <span><i class="flaticon-completed-task"></i></span>
                     </div>
                     <div class="it-funfact-content">
                        <h6><i class="purecounter" data-purecounter-duration="1" data-purecounter-end="300">300</i>+</h6>
                        <span>Classes Completed</span>
                     </div>
                  </div>
               </div>
               <div class="col-xl-3 col-lg-3 col-md-6 col-sm-6">
                  <div class="it-funfact-item d-flex align-items-center">
                     <div class="it-funfact-icon">
                        <span><i class="flaticon-customer-review"></i></span>
                     </div>
                     <div class="it-funfact-content">
                        <h6><i class="purecounter" data-purecounter-duration="1" data-purecounter-end="95">95</i>%</h6>
                        <span>Satisfaction Rate</span>
                     </div>
                  </div>
               </div>
               <div class="col-xl-3 col-lg-3 col-md-6 col-sm-6">
                  <div class="it-funfact-item d-flex align-items-center">
                     <div class="it-funfact-icon">
                        <span><i class="flaticon-class"></i></span>
                     </div>
                     <div class="it-funfact-content">
                        <h6><i class="purecounter" data-purecounter-duration="1" data-purecounter-end="25">25</i>k+</h6>
                        <span>Students Community</span>
                     </div>
                  </div>
               </div>
            </div>
         </div>
      </div>
   </div>
   <!-- funfact-area-end -->

   <!-- testimonial-area-start -->
   <div class="it-testimonial-area it-testimonial-ptb pt-180 it-sub-bg-none it-testimonial-bg fix p-relative"
      data-background="<?= base_url() ?>public/web/assets/img/testimonial/testimonial-bg.jpg">
      <div class="it-testimonial-shape-1 d-none d-xl-block">
         <img src="<?= base_url() ?>public/web/assets/img/testimonial/shape-2-1.png" alt="">
      </div>
      <div class="container">
         <div class="it-testimonial-title-wrap mb-60">
            <div class="row align-items-end">
               <div class="col-xl-6 col-lg-8 col-md-9 col-sm-8">
                  <div class="it-testimonial-title-box">
                     <span class="it-section-subtitle ">
                        <img src="<?= base_url() ?>public/web/assets/img/category/inner.svg" alt="">
                        testimonial
                     </span>
                     <h4 class="it-section-title">Transforming Careers for
                        <span class="p-relative z-index">Enduring
                           <svg class="title-shape-3" width="169" height="65" viewBox="0 0 169 65" fill="none"
                              xmlns="http://www.w3.org/2000/svg">
                              <path
                                 d="M73.9865 8.52241C79.0935 6.03398 83.1809 4.26476 89.5018 3.31494C94.8148 2.51659 100.239 2.08052 105.59 1.95274C121.035 1.5839 135.743 4.94481 147.12 9.7789C159.246 14.931 167.348 22.7171 166.701 31.8511C165.923 42.8363 151.983 52.0035 134.146 57.1364C110.893 63.8284 82.3457 64.1305 59.197 61.1289C38.1374 58.3982 12.2599 51.9446 4.20444 40.1836C-2.8133 29.9382 12.6851 18.2085 28.1538 11.6691C40.9733 6.24978 56.315 2.97602 71.4123 4.09034C82.5481 4.91227 93.8269 6.91079 103.074 10.0494C113.489 13.5844 120.759 18.7016 128.482 23.7722"
                                 stroke="var(--it-theme-1)" stroke-width="3" stroke-linecap="round" />
                           </svg>
                        </span>
                        Trading
                     </h4>
                  </div>
               </div>
               <div class="col-xl-6 col-lg-4 col-md-3 col-sm-4">
                  <div class="it-testimonial-arrow text-end"></div>
               </div>
            </div>
         </div>
         <div class="row">
            <div class="col-xl-12">
               <div class="it-testimonial-slide-wrap p-relative">
                  <div class="it-testimonial-slide-quote d-none d-xl-block">
                     <i class="fa-sharp fa-solid fa-quote-right"></i>
                  </div>
                  <div class="row align-items-center">
                     <div class="col-xl-2 col-lg-2 col-md-3">
                        <div class="it-testimonial-slide-nav-active text-center">
                           <div class="it-testimonial-nav-item">
                              <div class="it-testimonial-nav-thumb">
                                 <img src="<?= base_url() ?>public/web/assets/img/testimonial/khalid.png" alt="">
                              </div>
                           </div>
                           <div class="it-testimonial-nav-item">
                              <div class="it-testimonial-nav-thumb">
                                 <img src="<?= base_url() ?>public/web/assets/img/testimonial/maria.png" alt="">
                              </div>
                           </div>
                           <div class="it-testimonial-nav-item">
                              <div class="it-testimonial-nav-thumb">
                                 <img src="<?= base_url() ?>public/web/assets/img/testimonial/khami.png" alt="">
                              </div>
                           </div>
                           <div class="it-testimonial-nav-item">
                              <div class="it-testimonial-nav-thumb">
                                 <img src="<?= base_url() ?>public/web/assets/img/testimonial/musallam.png" alt="">
                              </div>
                           </div>
                           <div class="it-testimonial-nav-item">
                              <div class="it-testimonial-nav-thumb">
                                 <img src="<?= base_url() ?>public/web/assets/img/testimonial/aryna.png" alt="">
                              </div>
                           </div>
                        </div>
                     </div>
                     <div class="col-xl-9 col-lg-9 col-md-9">
                        <div class="it-testimonial-slide-active">
                           <div class="it-testimonial-slide-item">
                              <div class="it-testimonial-slide-info">
                                 <h4 class="it-testimonial-user-name">Khalid Butti Al Zaabi</h4>
                                 <span>happy customer</span>
                                 <p>“FXCareers Dubai transformed my financial journey from novice to professional. Their expert-led courses and supportive community gave me the confidence to navigate the Forex market with ease. Thanks to their comprehensive training, I've achieved financial independence and a thriving career I never thought possible.</p>
                              </div>
                           </div>
                           <div class="it-testimonial-slide-item">
                              <div class="it-testimonial-slide-info">
                                 <h4 class="it-testimonial-user-name">Maria Sakkari</h4>
                                 <span>Exceptional Training</span>
                                 <p>“The trainers at FXCareers Dubai are excellent and genuinely enjoy teaching. They make learning easy and enjoyable, and they simply explain complex topics. Thanks to FXCareers, I now have the knowledge and confidence to trade effectively.</p>
                              </div>
                           </div>
                           <div class="it-testimonial-slide-item">
                              <div class="it-testimonial-slide-info">
                                 <h4 class="it-testimonial-user-name">Khamis Al Mansoori</h4>
                                 <span>Game Changer</span>
                                 <p>“FXCareers Dubai showed me new opportunities I didn’t see before. The lessons were full of practical advice and real tips that made a big difference. Now I’ve got a clear plan and the strategies to tackle Forex with confidence. It’s like having a map to success!</p>
                              </div>
                           </div>
                           <div class="it-testimonial-slide-item">
                              <div class="it-testimonial-slide-info">
                                 <h4 class="it-testimonial-user-name">Musallam Fayez Al Hamdani</h4>
                                 <span>Great Trading Education Experience</span>
                                 <p>“Mashallah! FXCareers offers excellent learning in trading education. The trainers provided great support and encouraged us to ask questions to get clear answers. They teach us using demo accounts so we can see how trading works, and help us understand charts and graphs better. From being confused, I have become a skilled trader.</p>
                              </div>
                           </div>
                           <div class="it-testimonial-slide-item">
                              <div class="it-testimonial-slide-info">
                                 <h4 class="it-testimonial-user-name">Aryna Sabalenka</h4>
                                 <span>Best Forex Learning Platform</span>
                                 <p>“I found that FXCareers offers a lot of learning materials like webinars, articles, and videos. They cover everything important in Forex trading, such as reading charts, understanding the market, and managing risks. I highly recommend FXCareers as the best place to learn.</p>
                              </div>
                           </div>
                        </div>
                     </div>
                  </div>
               </div>
            </div>
         </div>
      </div>
   </div>
   <!-- testimonial-area-end -->

   <!-- course-area-start -->
   <div class="it-course-area it-sub-bg-none p-relative grey-bg pt-80 pb-80">
      <div class="it-course-shape-1 d-none d-xl-block">
         <img src="<?= base_url() ?>public/web/assets/img/course/shape-1-1.png" alt="">
      </div>
      <div class="it-course-shape-2 d-none d-xxl-block">
         <img src="<?= base_url() ?>public/web/assets/img/course/shape-1-2.png" alt="">
      </div>
      <div class="it-course-shape-3 d-none d-xl-block">
         <img src="<?= base_url() ?>public/web/assets/img/course/shape-1-3.png" alt="">
      </div>
      <div class="it-course-shape-4 d-none d-xl-block">
         <img src="<?= base_url() ?>public/web/assets/img/course/shape-1-4.png" alt="">
      </div>
      <div class="container">
         <div class="row">
            <div class="col-xl-12">
               <div class="it-course-title-box text-center mb-70">
                  <span class="it-section-subtitle">Most Popular Courses</span>
                  <h4 class="it-section-title">Top Study Courses
                     <span class="p-relative z-index">That students
                        <svg class="title-shape-2" width="168" height="65" viewBox="0 0 168 65" fill="none"
                           xmlns="http://www.w3.org/2000/svg">
                           <path
                              d="M73.3761 8.49147C78.4841 6.01353 82.5722 4.25154 88.8933 3.3035C94.2064 2.50664 99.6305 2.0701 104.981 1.94026C120.426 1.56549 135.132 4.90121 146.506 9.70405C158.628 14.8228 166.725 22.5638 166.074 31.6501C165.291 42.5779 151.346 51.7039 133.508 56.8189C110.253 63.4874 81.7065 63.8025 58.5605 60.8285C37.5033 58.123 11.6304 51.7165 3.58132 40.0216C-3.43085 29.8337 12.0728 18.1578 27.544 11.645C40.3656 6.24763 55.7082 2.98328 70.8043 4.08403C81.9391 4.89596 93.2164 6.87822 102.462 9.99561C112.874 13.5066 120.141 18.5932 127.862 23.6332"
                              stroke="var(--it-theme-1)" stroke-width="3" stroke-linecap="round" />
                        </svg>
                     </span>
                     can <br> join with us.
                  </h4>
               </div>
            </div>
            <?php foreach ($course_details_view as $row): ?>
               <div class="col-xl-4 col-lg-4 col-md-6 mb-30">
                  <div class="it-course-item">
                     <div class="it-course-thumb mb-20 p-relative">
                        <a
                           href="<?= base_url() ?>online-detail/<?= str_replace(' ', '-', strtolower($row->course_name)); ?>"><img
                              src="<?= base_url('uploads/course/') . $row->course_image; ?>" alt="image"></a>
                        <div class="it-course-thumb-text">
                           <span><?= $row->mode ?></span>
                        </div>
                     </div>
                     <div class="it-course-content">

                        <h4 class="it-course-title pb-5"><a
                              href="<?= base_url() ?>online-detail/<?= str_replace(' ', '-', strtolower($row->course_name)); ?>"><?= $row->course_name ?></a>
                        </h4>
                        <div class="it-course-info pb-15 mb-25 d-flex justify-content-between">
                           <span><i class="fa-light fa-file-invoice"></i>Lesson <?= $row->lesson ?></span>
                           <span><i class="fa-sharp fa-regular fa-clock"></i><?= $row->duration ?></span>

                        </div>

                        <div class="it-course-price-box d-flex justify-content-between">
                           <span><i><?= $row->price ?> AED</i></span>
                           <a
                              href="<?= base_url() ?>online-detail/<?= str_replace(' ', '-', strtolower($row->course_name)); ?>"><i
                                 class="fa-light fa-cart-shopping"></i>Buy Course</a>
                        </div>
                     </div>
                  </div>
               </div>
            <?php endforeach; ?>


         </div>
         <div class="col-xl-12">
            <div class="it-course-button text-center pt-45">
               <a class="it-btn" href="<?= base_url() ?>courses">
                  <span>
                     More Courses
                     <svg width="17" height="14" viewBox="0 0 17 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M11 1.24023L16 7.24023L11 13.2402" stroke="currentcolor" stroke-width="1.5"
                           stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round" />
                        <path d="M1 7.24023H16" stroke="currentcolor" stroke-width="1.5" stroke-miterlimit="10"
                           stroke-linecap="round" stroke-linejoin="round" />
                     </svg>
                  </span>
               </a>
            </div>
         </div>
      </div>
   </div>
   </div>
   <!-- course-area-end -->

   <!-- team-area-start -->
   <div class="it-team-3-area p-relative z-index pt-80 pb-80">
      <div class="it-team-3-bg" data-background="<?= base_url() ?>public/web/assets/img/team/bg-4.jpg"></div>
      <div class="container">
         <div class="row">
            <div class="col-xl-12">
               <div class="it-event-title-box text-center pb-40">
                  <span class="it-section-subtitle text-white">
                     <img src="<?= base_url() ?>public/web/assets/img/team/bok.svg" alt="">
                     Mentors
                     <img src="<?= base_url() ?>public/web/assets/img/team/bok.svg" alt="">
                  </span>
                  <h2 class="it-section-title-3 text-white">meet our expert mentors</h2>
               </div>
            </div>
         </div>
         <div class="row">
            <div class="col-xl-3 col-lg-4 col-md-6 mb-30">
               <?php foreach ($teachers as $row): ?>
                  <div class="it-team-3-item text-center">
                     <div class="it-team-3-thumb fix">
                        <img src="<?= base_url() ?>uploads/teachers/<?= $row->teacher_image ?>" alt="">
                     </div>
                     <div class="it-team-3-content">
                        <!-- <div class="it-team-3-social-box p-relative">
                           <button>
                              <i class="fa-light fa-share-nodes"></i>
                           </button>
                           <div class="it-team-3-social-wrap">
                              <a href="#"><i class="fa-brands fa-instagram"></i></a>
                              <a href="#"><i class="fa-brands fa-facebook-f"></i></a>
                              <a href="#"><i class="fa-brands fa-pinterest-p"></i></a>
                              <a href="#"><i class="fa-brands fa-linkedin-in"></i></a>
                           </div>
                        </div> -->
                        <div class="it-team-3-author-box">
                           <h4 class="it-team-3-title"><a
                                 href="<?= base_url() ?>mentor/<?= str_replace(' ', '-', strtolower($row->teacher_name)); ?>"><?= $row->teacher_name ?></a>
                           </h4>
                           <span><?= $row->designation ?></span>
                        </div>
                     </div>
                  </div>
               <?php endforeach ?>
            </div>

         </div>
      </div>
   </div>
   <!-- team-area-end -->

<!-- faq section -->
<?php include("include/faq.php") ?>

</main>